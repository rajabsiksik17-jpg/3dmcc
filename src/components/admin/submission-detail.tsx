      ...
      <div className="lg:col-span-2">
        <div className="rounded-2xl border border-charcoal-100 bg-white shadow-card">
          <div className="border-b border-charcoal-100 px-6 py-4">
            <h2 className="font-semibold text-charcoal-900">{t("submissionDetails")}</h2>
          </div>
          <dl className="divide-y divide-charcoal-100">
            <div className="grid grid-cols-2 gap-2 px-6 py-3">
              <dt className="text-sm text-charcoal-500">{t("type")}</dt>
              <dd className="text-sm font-medium text-charcoal-900">{t(submission.form_type) || submission.form_type}</dd>
            </div>
            {values.map((v) => (
              <div key={v.id} className="grid grid-cols-2 gap-2 px-6 py-3">
                ...
                <dd className="text-sm text-charcoal-900 break-words">{formatValue(v.value)}</dd>
              </div>
            ))}
            <div className="grid grid-cols-2 gap-2 px-6 py-3">
              <dt className="text-sm text-charcoal-500">{t("submitted")}</dt>
...
function NotesEditor({ submissionId, initial }: { submissionId: string; initial: string }) {
  ...
  return (
    <div>
      ...
      <Button size="sm" className="mt-2" loading={pending} onClick={save}>{t("saveNote")}</Button>
    </div>
  );
}
