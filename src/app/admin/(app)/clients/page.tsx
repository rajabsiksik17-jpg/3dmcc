...
import { adminClients } from "@/lib/admin-data";
import { saveClient, deleteClient } from "@/app/admin/actions/content";
...
export default async function ClientsPage({ searchParams }: { searchParams: Promise<{ edit?: string }> }) {
  await requireAdmin();
  const { t } = await getAdminT();
  const { edit } = await searchParams;
  const clients = await adminClients();
  const editing = edit && edit !== "new" ? clients.find((c) => c.id === edit) : undefined;

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold text-charcoal-900">{t("clients")}</h1>
        <Link href="/admin/clients?edit=new" className="btn-primary !py-2">{t("new")}</Link>
      </div>
              ...
              <tr key={c.id} className="hover:bg-charcoal-50">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <Link href={`/admin/clients?edit=${c.id}`} className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-brand-600 hover:bg-brand-50">{t("edit")}</Link>
